  ...
  function () public {
    assembly {
      ...
      if eq(sig, 0x5a984ded /*   function frob(bytes32 ilk, int256 dink, int256 dart) external; */) {
        ...
        if iszero(call(gas, sload(2), 0, 0, 196, 0, 0)) { revert(0, 0) }
        ...
        if iszero(call(gas, sload(2), 0, 0, 36, 0, 128)) { revert(0, 0) }
        ...
        if iszero(call(gas, sload(2), 0, 0, 68, 0, 64)) { revert(0, 0) }
        ...
        // iff vat.call("debt()") != 0
        if iszero(call(gas, sload(2), 0, 0, 4, 0, 32)) { revert(0, 0) }
        ...
        if iszero(and(and(or(calm, cool), or(and(cool, firm), safe)), sload(4))) { revert(0, 0) }

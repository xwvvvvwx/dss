  ...
  function () public {
    assembly {
      ...
      if eq(sig, 0xbf353dbb /*   function wards(address guy) external returns (bool can); */) {
        let hash_0 := hash2(10, calldataload(4))
      ...
      if eq(sig, 0x29ae8114 /*   function file(bytes32 what, uint256 data) external; */) {
        ...
        if eq(calldataload(4), "wait") { sstore(7, calldataload(36)) }

        // if what == "sump" set sump = data
        if eq(calldataload(4), "sump") { sstore(8, calldataload(36)) }

        // if what == "bump" set bump = data
        if eq(calldataload(4), "bump") { sstore(9, calldataload(36)) }

        // if what == "hump" set hump = data
        if eq(calldataload(4), "hump") { sstore(10, calldataload(36)) }
      ...
      if eq(sig, 0x697efb78 /*   function fess(uint256 tab) external; */) {
        ...
        // set Sin += tab
        sstore(4, uadd(sload(4), calldataload(4)))
      ...
      if eq(sig, 0x35aee16f /*   function flog(uint48 era_) external; */) {
        ...
        // iff era_ + wait <= era()
        if gt(uadd(calldataload(4), sload(7)), era()) { revert(0, 0) }
        ...
        // set Sin -= sin_era_
        sstore(4, usub(sload(4), sin_era_))

        // set Woe += sin_era_
        sstore(5, uadd(sload(5), sin_era_))
      ...
      if eq(sig, 0xbbbb0d7b /*   function flop() external returns (uint256); */) {
        ...
        // set Woe -= sump
        sstore(5, usub(Woe_, sump))

        // set Ash += sump
        sstore(6, uadd(sload(6), sump))
      ...
      if eq(sig, 0x0e01198b /*   function flap() external returns (uint256); */) {
        ...
        if lt(Joy(), uadd(uadd(Awe(), bump), sload(10))) { revert(0, 0) }
        ...
        // iff cow.call("dai()") != 0
        if iszero(call(gas, sload(1), 0, 0, 4, 64, 32)) { revert(0, 0) }
      ...
      function Awe() -> wad {
        wad := uadd(uadd(sload(4), sload(5)), sload(6))
      }
      function Joy() -> wad {
      ...
      function pleb() -> x {
        x := iszero(eq(1, sload(hash2(10, caller))))
      }
      // map[key] translates to hash(key ++ idx(map))
      function hash2(b, i) -> h {
        mstore(0, i)
